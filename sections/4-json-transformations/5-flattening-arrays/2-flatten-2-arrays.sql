...
select to_variant('{ cities: [
    { city: "Vancouver", persons: [{ person: "Mark", age: 55}, {person: "John", age: 80 }] },
    { city: "Toronto", persons: [{person: "Mary", age: 20}, {person: "George", age: 44}] },
    { city: "Montreal", persons: [{ person: "Jane", age: 30}] }
  ]}');
...
-- no results
with x as (select parse_json(v) as json from cities)
...
with x as (select parse_json(v) as json from cities),
y as (
    select value:city::string as city, value:persons as persons
